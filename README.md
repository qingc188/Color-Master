...
每个难度共 10 轮。每轮将目标色和复现色转换到 Oklab，以感知色差和分段评分曲线计分，满分 10 分，总分满分 100 分。结果页使用简短的定性反馈；需要查看具体色差、评分区间和锚点时，可打开得分旁的评分说明。
...
Oklab 评分是第二版评分标准。旧版 RGB 距离成绩与新版分数不可直接比较，因此新版使用独立的 LocalStorage 记录键，不会把旧成绩误当成新版最佳成绩。
...
成绩和色彩历史保存在当前浏览器的 LocalStorage 中。更换浏览器、清理站点数据或更换设备后，记录不会自动同步。如果所在 WebView 禁止本地存储，游戏仍可完成当前一局，并会提示本次记录无法保存。
...
测试覆盖 Oklab 参考向量、RGB 往返转换、评分锚点、得分单调性、色板唯一性和各难度感知距离范围。
