...
每个难度共 10 轮。每轮将目标色和复现色转换到 Oklab；相比直接比较屏幕的 RGB 数字，它更接近人眼对明度、饱和度和色相变化的感受。系统再以感知色差和分段评分曲线计分；当明显带色的目标被复现为过低饱和度时，会增加低饱和度修正。满分 10 分，总分满分 100 分。结果页会用色相、饱和度和明度说明主要偏差。
...
当前评分是第四版标准，在 Oklab 基础色差上增加目标导向的低饱和度修正，避免明度接近但饱和度差异明显时获得过高分。第四版使用独立的 LocalStorage 记录键，不会把旧版成绩误当成新版最佳成绩。
...
浏览器回归会检查基础匹配色板、防重复提交、准备页与进行中统计栏、HSL/RGB 键盘操作、复现反馈与复盘指导语、存储和音频失败降级、持久化文本安全、手机首屏提交，以及 200% 文字缩放下的滚动和横向溢出。使用其他 Chromium 浏览器时，可通过 `BROWSER_PATH` 指定可执行文件。
