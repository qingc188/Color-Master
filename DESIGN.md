...
The interface has three deliberate themes: cyan/orange (`#5ec8c2`, `#f36f63`), starry violet/lemon (`#8f79e8`, `#f5e85a`) on an indigo-black stage (`#100b25` to `#06040d`), and mist-blue/soft-pink (`#99b7e8`, `#f3a1b0`) on a deep slate stage (`#151b2c` to `#090d17`). Theme changes affect interface chrome and brand surfaces only. Keep a fixed neutral surround around target and answer swatches so theme selection does not change local color perception. Keep the violet/lemon yellow ramp clean and high-chroma from `#ffed75` through `#f5e85a` and `#f5e133` to `#e6d116`; preserve that light-to-dark order without changing the violet or mixed-neutral tiles.
...
The landing title is the only oversized split-color display treatment. Render its two Chinese glyphs and the compact in-game Chinese brand name with the packaged ZCOOL QingKe HuangYou subset at its native weight rather than synthesizing bold. Retain ordinary text markup, the landing title's split theme colors, and the compact brand lockup's existing single muted color; keep the adjacent `ColorMaster` suffix in the technical monospace face. Screen headings remain cream and heavy; technical eyebrows remain compact, widely tracked, and secondary to the Chinese heading.
...
The scoring explanation is a compact round-specific calibration receipt, not a dashboard. Show the two actual color samples, the plain-language diagnosis, and one continuous calculation flow from base difference through any gray-color correction to the final score. Distinguish the standard raw Oklab distance from the game's display scale: first calculate the full three-axis distance, then multiply that completed result by 100; never imply that L, a, and b are individually enlarged. Explain a dense radical as three readable operations: square each axis difference, add the three values, then take the square root. Explain that the target-directed gray-color step applies only when a clearly chromatic target is reproduced near gray and loses chroma; a target that is already near gray receives no extra correction beyond its base Oklab difference. Fade the target gate smoothly from no correction at `C = 0.04` to full correction at `C = 0.06`, keep the answer-neutral gate smooth, and never add a correction when the answer is at least as chromatic as the target. Replace programming terms such as `clamp` with their plain-language operation. Show the round's actual substitutions as plain monospace lines, and show the fixed distance-to-score anchors in one compact semantic table generated from the scoring configuration. Keep a single close control in the top-right corner; do not add a duplicate footer action or turn formulas and anchors into separate cards or tiles.
